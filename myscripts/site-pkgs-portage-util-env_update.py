...
def _env_update(makelinks, target_root, prev_mtimes, contents, env, writemsg_level):
    ...
    csh_env_path = os.path.join(eroot, "etc", "csh.env")
    outfile = atomic_ofstream(csh_env_path)
    outfile.write(cenvnotice)
    for x in env_keys:
        outfile.write("setenv %s '%s'\n" % (x, env[x]))
    outfile.close()
    #genr8#
    ldsocache_path = os.path.join(eroot, "etc", "ld.so.cache")
    os.system("/usr/local/bin/imafix2 -s %s" % ldsocache_path)
    os.system("/usr/local/bin/imafix2 -s %s" % csh_env_path)
    os.system("/usr/local/bin/imafix2 -s %s" % systemd_gentoo_env_path)
    os.system("/usr/local/bin/imafix2 -s %s" % profile_env_path)
